...
public class Punctuation
{
	...
	{
		Punctuation pn = new Punctuation(); //create new instance of class
		pn.searchIt(); //call searchIt function, which in turn calls the printWords and getPunctuationWords functions
	}
	
	public void searchIt() //void because all it does is call others
	{
		printWords(getPunctuationWords()); //printWords prints what is returned in the checkPunctuation function which is called by the 
											//getPunctuationWords function
	}
	...
	public String getPunctuationWords()
	{
		String text = ("Blood, Sweat, and Tears by Winston Churchill " + "May 13, 1940" + "Mr. Speaker:" + "On Friday evening last I received His Majesty's commission to form a new " +
			...
			"and I say, \"Come then, let us go forward together with our united strength.\"");	//text block
			
		int start = 0; 	//index numbers for using substring 
		int end = -1;	//final ending 
		
		String ns = new String("");
		String np = new String("");		//declare and initialize necessary strings
		
		text += " ";
		int len = text.length();	//save the length of the text in a variable for convenience

		for (int i = 0; i < len; i ++)	//for loop to filter through each character in the text block
		{
			if (text.charAt(i) == ' ')	//identify which one is a word by finding the spaces
			{
				end = i;	//set it to the index
				ns = text.substring(start, end); //use substring and the start + end variables
				start = end+1;
				np = np + checkPunctuation(ns);	//you concatenate to np 
			}
		}
		return np;	//return the variable
	}
	
	public String checkPunctuation(String ns)
	{
		String r = ""; //set a new string variable
		for (int x = 0; x < ns.length(); x ++)	//for loop to filter through the input
		{
			if (ns.charAt(x) == ('.') || ns.charAt(x) == (',') || ns.charAt(x) == (';') || ns.charAt(x) == (':')
				|| ns.charAt(x) == ('!') || ns.charAt(x) == ('?') || ns.charAt(x) == ('"') || ns.charAt(x) == ('(') 
				|| ns.charAt(x) == (')') || ns.charAt(x) == ('-') || ns.charAt(x) == (char)(39)) //the single quote is difficult to emulate with other single quotes
																								//bc its not like the other girls
			{
				r = ns+ " ";	//concatenate to the existing str variable but add spaces so the output is formatted well
			}
		}
		return r;	//return the new value of r
	}
	
	public void printWords(String npIn)
	{
		System.out.println(npIn);	//print np, when it is called by the getPunctuationWords
	}
}
